...
$query = "SELECT * FROM LOAN_TYPE ORDER BY loan_type_id DESC";
...
<head>
  <meta charset="UTF-8" />
  <title>Loan Type Management</title>
...
<nav class="sidebar" id="sidebar">
  ...
  <main class="main-content">
      ...
      <tbody>
        <?php while ($row = $result->fetch_assoc()): ?>
          <tr>
            ...
            <td><?= ucfirst($row['interest_type']) ?></td>
            <td>
            <a href="#" class="action-btn view-btn"
            data-id="<?= $row['loan_type_id'] ?>"
            data-name="<?= htmlspecialchars($row['name']) ?>"
            data-rate="<?= number_format($row['interest_rate'], 2) ?>"
            data-type="<?= ucfirst($row['interest_type']) ?>">View</a>
                |
              <a href="edit_Loan.php?loan_type_id=<?= $row['loan_type_id'] ?>">Edit</a> |
              <a href="delete_Loan.php?loan_type_id=<?= $row['loan_type_id'] ?>" onclick="return confirm('Delete this loan type?')">Delete</a>
            </td>
          </tr>
        <?php endwhile; ?>
      </tbody>
    ...
    <a href="add_Loan.php" class="login-btn">Add New Loan Type</a>
  ...
  <div style="background:#fff; color:#000; margin:10% auto; padding:20px; width:400px; border-radius:10px; position:relative;">
    <span id="closeModalBtn" style="position:absolute; top:10px; right:15px; cursor:pointer; font-size:18px;">&times;</span>
        ...
        <p><strong>Loan Type ID:</strong> <span id="view-id"></span></p>
        <p><strong>Loan Name:</strong> <span id="view-name"></span></p>
        <p><strong>Interest Rate:</strong> <span id="view-rate"></span>%</p>
        <p><strong>Interest Type:</strong> <span id="view-type"></span></p>
    </div>
    </div>
