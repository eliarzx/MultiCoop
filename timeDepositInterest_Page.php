...
$query = "SELECT * FROM TIME_DEPOSIT_INTEREST ORDER BY created_at DESC";
...
<head>
  <meta charset="UTF-8" />
  <title>Time Deposit Interest Management</title>
...
<main class="main-content">
  <h1>Manage Time Deposit Interests</h1>
  <table>
    <thead>
      <tr>
        <th>Interest ID</th>
      ...
      <tr>
        ...
        <td>
          <a href="#" class="view-btn"
             ...
             data-rate="<?= number_format($row['interest_rate'], 2) ?>"
             data-date="<?= $row['created_at'] ?>">View</a>
          |
          <a href="edit_timeDepositInterest.php?interest_id=<?= $row['interest_id'] ?>">Edit</a>
          |
          <a href="delete_timeDepositInterest.php?interest_id=<?= $row['interest_id'] ?>"
             onclick="return confirm('Are you sure you want to delete this interest record?');">Delete</a>
        </td>
      </tr>
      <?php endwhile; ?>
    </tbody>
  ...
  <a href="add_timeDepositInterest.php" class="login-btn">Add New Interest</a>
...
<div id="viewModal" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.6);">
  <div style="background:#fff; color:#000; margin:10% auto; padding:20px; width:400px; border-radius:10px; position:relative;">
    ...
    <h2>Time Deposit Interest Details</h2>
    <p><strong>Interest ID:</strong> <span id="view-id"></span></p>
    <p><strong>Term (Months):</strong> <span id="view-term"></span></p>
...
document.querySelectorAll(".dropdown-btn").forEach(btn => {
  ...
  document.querySelectorAll(".view-btn").forEach(button => {
    button.addEventListener("click", () => {
      ...
      document.getElementById("view-term").textContent = button.dataset.term;
