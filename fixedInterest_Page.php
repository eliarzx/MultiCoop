...
$query = "SELECT FI.*, FA.account_id 
          FROM FIXED_ACCOUNT_INTERESTS FI
          JOIN FIXED_ACCOUNTS FA ON FI.account_id = FA.account_id
          ORDER BY FI.interest_date DESC";
...
<head>
  <meta charset="UTF-8" />
  <title>Fixed Account Interest Management</title>
...
<nav class="sidebar" id="sidebar">
  ...
  <main class="main-content">
    <h1>Manage Fixed Account Interests</h1>
    <table>
      <thead>
        <tr>
          <th>Interest ID</th>
        ...
        <tr>
          ...
          <td>
            <a href="#" class="action-btn view-btn"
                ...
                data-date="<?= $row['interest_date'] ?>">View</a>
            |
            <a href="edit_fixedInterest.php?interest_id=<?= $row['interest_id'] ?>" class="action-btn">Edit</a>
            |
            <a href="delete_fixedInterest.php?id=<?= $row['interest_id'] ?>"
                class="action-btn"
    ...
    <a href="add_fixedInterest.php" class="login-btn">Add New Interest</a>
  ...
  <div style="background:#fff; color:#000; margin:10% auto; padding:20px; width:400px; border-radius:10px; position:relative;">
    <span id="closeModalBtn" style="position:absolute; top:10px; right:15px; cursor:pointer; font-size:18px;">&times;</span>
        <h2>Fixed Account Interest Details</h2>
        <div class="modal-body">
        <p><strong>Interest ID:</strong> <span id="view-id"></span></p>
        <p><strong>Account ID:</strong> <span id="view-account_id"></span></p>
        <p><strong>Interest Amount:</strong> ₱<span id="view-amount"></span></p>
        <p><strong>Interest Date:</strong> <span id="view-date"></span></p>
        </div>
    </div>
    </div>
