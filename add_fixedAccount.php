...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("INSERT INTO FIXED_ACCOUNTS (member_id, account_balance, deposit_date, maturity_date, interest_rate, status) VALUES (?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("idssdi", $member_id, $account_balance, $deposit_date, $maturity_date, $interest_rate, $status);

    if ($stmt->execute()) {
        echo "<script>alert('Fixed account added successfully.'); window.location.href='fixedAccounts_Page.php';</script>";
        exit();
    } else {
        echo "<script>alert('Failed to add fixed account.');</script>";
    }
}
    ...
    <div class="form-container">
        ...
        <form method="POST">
            ...
            <div class="form-actions">
                <button type="submit" class="add-btn">Add Fixed Account</button>
